...
describe('Unit test of json rpc client',function(){
  ...
  beforeEach(function(){
    ...
    var createResponse = function(rpc) {
      ...
      if (rpc.method === "error") {
        ...
        result.result = { id: rpc.id,msg: "foobar",xhr: true}; //send id so its easy to test response
      }
      return result;
    };
    ...
    it('should use WebSockets when available',function(done){
      ...
      var client = new $.JsonRpcClient({
        ...
        onmessage: function(result){
          expect(result.data).to.be.equal('{"jsonrpc":"2.0","method":"foobar","params":[],"id":1}');
          done();
        }
      });
  ...
  it('should send along messages that fail JSON parsing to fallback message handler',function(done){
    var dontCall = sinon.stub().throws();
